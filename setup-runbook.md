...
- **Mitigation A (preferred):** Cap V620 power with `scripts/66-v620-powercap.sh` (default 180 W per card; measured −7% prefill, no generation loss, junction max 86°C vs 101°C at the stock 250 W). Monitor edge/junction temperatures with `pct exec 151 -- rocm-smi --showtemp` during sustained load. Do **not** use `rocm-smi --setpoweroverdrive` (the VBIOS pins the range at 250–250 W, so any lower value is refused) or `--setperflevel low` (it selects the 0 MHz sleep state and wedged the tensor-split chat server on 2026-09-11). See [day-2-ops § 3](day-2-ops.md#-4-model-management) — *Power cap*.
