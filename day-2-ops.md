...
**Check the proactive restart timer first.** The most common non-obvious cause is
`llamacpp-chat-restart.timer` on LXC 151 restarting the chat unit underneath a live
request — see [§ 3.11](#-311-chat-restarts-itself-proactive-restart-timer). It costs one
command and rules out the whole class:

```bash
# Did the timer fire around the time of the failure?
pct exec 151 -- systemctl show llamacpp-chat-restart.timer -p LastTriggerUSec
```

Note the client sees only the inner error object, which looks like a provider outage
rather than anything local:

```json
{"message":"upstream returned an error; retry shortly","type":"service_degraded","code":null}
```
...
### § 3.11 Chat restarts itself (proactive restart timer)

**Symptom:** clients get `service_degraded` — or a stream simply dies — with no crash in
the journal, no failed unit, and `NRestarts=0`. Health checks pass by the time you look.

**What happened on 2026-09-09:** `llamacpp-chat-restart.timer` fired at 16:10:05 UTC into
an active ~60K-token generation. llama.cpp would not drain, systemd hit the default
`TimeoutStopSec=90s` and SIGKILLed it at 16:11:35, and chat was unavailable until 16:11:52
— **~107 s** for a restart that takes ~16 s when the slot is idle. Six research subagents
launched shortly after all failed against the dead upstream.

**Cause:** the timer used to run `systemctl restart llamacpp-chat` *unconditionally* at two
fixed times a day. Its `RandomizedDelaySec` was documented as softening the impact on
in-flight requests — it does not. Jitter relocates the collision; it does not reduce its
probability. With one long-running user on a single-slot chat server, a twice-daily blind
restart lands mid-request eventually.

**Fix (in place since 2026-09-09):** the timer now fires `hourly` and delegates the
decision to [`chat-restart-if-idle.sh`](./scripts/files/chat-restart-if-idle.sh), installed
by [`59-llamacpp-restart-timer.sh`](./scripts/59-llamacpp-restart-timer.sh). The guard
restarts only in a genuine lull, and rate-limits itself to one restart per 12 h — so the
effective cadence is still ~2/day, but never on top of live work.

It skips when a slot is processing, when the router reports chat traffic inside
`IDLE_MIN`, while the unit is still warming, and when `/slots` answers with any non-200
status (a `401` from a rotated key means the server is *alive*, not wedged — restarting
there would recreate this same outage during [§ 7](#-7-secrets--key-rotation) key
rotation). It restarts only when `/slots` gives no HTTP response at all after a retry, or
when the slot is genuinely idle. The rate-limit stamp is written only after the unit is
confirmed back up, so a failed restart does not suppress recovery for 12 h.

**Diagnose:**

```bash
# Did the timer fire near the failure?
pct exec 151 -- systemctl show llamacpp-chat-restart.timer -p LastTriggerUSec

# What did the guard decide, and why?
pct exec 151 -- journalctl -u llamacpp-chat-restart.service --since '24 hours ago'

# Next scheduled evaluation (not necessarily a restart)
pct exec 151 -- systemctl list-timers llamacpp-chat-restart.timer --no-pager
```

**Tunables** — `/etc/llamacpp-chat-restart.env` on LXC 151, regenerated by the installer:

| Variable | Default | Meaning |
|---|---|---|
| `IDLE_MIN` | `900` | Seconds of no chat traffic before the slot counts as free |
| `MIN_INTERVAL` | `43200` | Minimum seconds between proactive restarts (12 h) |
| `MIN_UPTIME` | `120` | Unit must be up this long to be eligible (skips warm-up) |
| `HEALTHZ_URL` | router `/healthz` | Session-liveness source; if unset the guard **skips** rather than guess |

Installer overrides: `LLAMA_RESTART_ONCALENDAR`, `LLAMA_RESTART_IDLE_MIN`,
`LLAMA_RESTART_MIN_INTERVAL`, `LLAMA_RESTART_MIN_UPTIME`, `LLAMA_CHAT_STOP_TIMEOUT`.
`LLAMA_RESTART_SCHEDULE` is **deprecated and ignored** — pinning fixed times would defeat
the gate.

**Operate it:**

```bash
# Ask the guard to evaluate now (it will skip if the slot is in use)
pct exec 151 -- systemctl start llamacpp-chat-restart.service

# Bypass the gate entirely (this WILL interrupt an in-flight request)
pct exec 151 -- systemctl restart llamacpp-chat
```

**Is the restart still needed?** Its original justification was a `--cache-reuse`
`hipMemcpyAsync` fault on gfx1030 firing ~6×/24 h. The qwen3.8 profile logs
`cache_reuse is not supported by this context, it will be disabled` on every start (the MTP
draft context disables it), and no in-session ROCm fault appeared in 6 days / 20 starts
when this was reviewed. The timer is kept for its wedged-server recovery path, which is
cheap now that it is gated. Revisit if the profile changes.

**Related:** `TimeoutStopSec=20s` is applied to `llamacpp-chat.service` via a drop-in at
...
The Tavily free tier is 1000 searches/month. Each external HTML reporting artifact refresh uses ~10 credits.
...
The router can run the OpenAI tools/tool_calls multi-turn loop internally instead of returning `tool_calls` to the client. Browser-side clients (the external HTML reporting artifact, ad-hoc curl scripts) get tool-augmented chat without implementing a dispatcher.
...
#      external HTML artifact or wherever the key is stored
