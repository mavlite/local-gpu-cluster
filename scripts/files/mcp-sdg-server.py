...
def _make_query_tool(workspace: str, description: str):
    ...
    fn.__doc__ = (
        f"SLOW / human-facing only. Full RAG SYNTHESIS on the '{workspace}' "
        f"workspace (embed → retrieve → rerank → the chat model generates "
        f"prose). This runs a second LLM over a large retrieved context on a "
        f"single chat slot; measured latency can exceed several MINUTES and "
        f"will time out most MCP clients (~120s).\n\n"
        f"AGENTS: do NOT use this — call search_{_tool_suffix(workspace)} "
        f"instead. It returns the same ranked source chunks in ~5s with no "
        f"generation; you (an LLM) synthesize from them yourself for a faster, "
        f"better-cited answer. Reserve query_ for human/UI callers that want a "
        f"finished paragraph and can tolerate the latency.\n\n"
