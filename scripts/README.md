...
| 12.8  | `66-v620-powercap.sh`   | Boot-time V620 power cap below the VBIOS 250 W floor (default 180 W). Oneshot unit before `pve-guests` writes a soft PowerPlay table ([`files/v620-powercap-apply.sh`](./files/v620-powercap-apply.sh), two driver-side bytes, RAM only) and sets `power1_cap`. Adds `amdgpu.ppfeaturemask=0xfff73fff` (GFXOFF off) to GRUB — the table write can hard-lock Navi 21 with GFXOFF on; a reboot is needed the first time |
...
| 13.1  | `71-vm-se-qa-firewall.sh` | PVE firewall policy confining VM 170 while it is on `vmbr0` — the documented fallback from `docs/se-qa-vm-requirements.md` §5, taken because the zero-egress question needs a rendering client the guest does not have. Denies the LAN (`192.168.6.0/24`, this host + every inference LXC) and the fleet (`10.60.0.0/16`), excepts DHCP renewal to the gateway, allows the rest so Steam works. **Separate from `70-` on purpose:** it flips the cluster-wide datacenter switch, so it guards first that no other guest has `firewall=1` and would start being filtered, and verifies afterwards via `fwbr*` interfaces rather than config flags. Leaves the host itself open — cluster-monitor `:8888` is probed to prove it |
