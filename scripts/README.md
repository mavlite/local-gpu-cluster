...
|------|----------------|-------|------|------|
| V620 LXC 151 | `llamacpp-chat.service` (both V620s, tensor-split per profile) | **Profile-switched** via [`swap-chat-model.sh`](./swap-chat-model.sh): `qwen3.6` (Qwen3.6-35B-A3B UD-Q4_K_M, 256K ctx, `tensor-split 1,1`, `cache-reuse 1024`, ~22 GB — default), `qwen3.6-hi` (UD-Q5_K_M, 256K ctx, `tensor-split 1,1.5`, `cache-reuse 1024†`, ~26.5 GB), or `coder` (Qwen3-Coder-Next UD-IQ4_XS, 128K ctx, `tensor-split 1,1.5`, `cache-reuse 0`, ~38 GB). Common flags: q8_0 KV, `--cache-ram 16384 --mlock --log-prefix --reasoning-format deepseek --jinja --no-mmproj`. †llama.cpp auto-disables cache_reuse for the Q5_K_M context | ~22 GB (qwen3.6) / ~26.5 GB (qwen3.6-hi) / ~38 GB (coder) | 8080 |
