...
> **Archived — superseded topology.** This describes a "2× V620 + 1× RTX 3060" hybrid with
> per-card model assignment and no cross-card tensor split. That hybrid was abandoned: the cluster
> is V620-only and does split across both cards. See §1.3 of
> [`../../local-gpu-cluster-v2.md`](../../local-gpu-cluster-v2.md) for the rationale. Kept for the
> cooling/power work and the staged single-card rollout procedure.
