...
> **Archived — never executed as written.** This was a plan to migrate the v1 host
> (`192.168.6.110`, 3× RTX 3060) off Ollama in place. It was overtaken by the v2 rebuild on new
> hardware, where llama.cpp was installed fresh rather than migrated. Kept for the parity-test
> design, the rollback staging, and the llama-swap/no-think proxy notes. For the current build see
